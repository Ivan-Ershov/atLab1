...
public class CreateCommandRecognizer {
    private final CreateCommandRecognizerContext stateMachine =
            new CreateCommandRecognizerContext(this);
    ...
    public List<String> check(String string)
    {
        ...
        for (int iterator = 0; iterator < length; iterator++) {
            ...
            boolean isOpenParenthesis = (current == '(');
            if (isOpenParenthesis) {
                stateMachine.openParenthesis();
                continue;
            }

            boolean isCloseParenthesis = (current == ')');
            if (isCloseParenthesis) {
                stateMachine.closeParenthesis();
                continue;
            }

            boolean isComma = (current == ',');
            if (isComma) {
                stateMachine.comma();
                continue;
            }
